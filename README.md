# NanaGraphy Website (with Aventa inside it)

The site root (`index.html`, this file's folder) is NanaGraphy's own
landing page — a simple one-page showcase of her photography and
editing work. Everything under `/aventa` is the full Aventa booking
site (packages, gallery, checkout, gifting) — a NanaGraphy product,
living at `yourdomain.com/aventa`. Both share the same design system
(`css/style.css`, `js/main.js`) and the same admin photo page
(`/admin`), which manages photos for both sections in one place.
...
emails, and a few for the admin photo-upload page. You can open any
`.html` file directly in a browser to preview it right now — the
...
button. Pick any JPEG, PNG, WebP, or HEIC (the default format on recent
iPhones) straight from a phone camera — the page
